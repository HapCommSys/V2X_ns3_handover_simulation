...
NS_LOG_COMPONENT_DEFINE ("RelatedWorkTesting");
...
{
  // std::string traceFile = "/home/yizhou/桌面/SUMODEMO/SimpleDemo/RelatedWorkTesting/2/traceFile.txt";
...
//   Config::SetDefault ("ns3::ThreeGppAntennaArrayModel::IsotropicElements", BooleanValue (true));
  ...
  mmwaveHelper->SetUePhasedArrayModelAttribute("NumRows", UintegerValue(10));
  ...
  // uint16_t length = 400;
  // uint16_t diff = 25;
  uint16_t nMmWaveEnbNodes = 3; // length / diff /2;
  ...
  enbPositionAlloc->Add (Vector (20, -10, 10));
  enbPositionAlloc->Add (Vector (50, 10, 10));
  enbPositionAlloc->Add (Vector (80, -10, 10));
  // for (uint32_t u = 0; u  < mmWaveEnbNodes.GetN(); ++u)
  //   {
  //     enbPositionAlloc->Add (Vector (u * 50 + diff, y_center + std::pow (-1, u) * 12, 10));
  //   }
