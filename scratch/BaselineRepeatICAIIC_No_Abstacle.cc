...
NS_LOG_COMPONENT_DEFINE ("BaselineRepeatICAIIC_No_Abstacle");
...
main (int argc, char *argv[])
{
  ...
  double ue_txPower = 1;
  double rsu_txPower = 40;
  cmd.AddValue ("ue_txPower", "Transmission power of each UE", ue_txPower);
  cmd.AddValue ("rsu_txPower", "Transmission power of each RSU", rsu_txPower);
  cmd.Parse (argc, argv);
...
//   Config::SetDefault ("ns3::ThreeGppAntennaArrayModel::IsotropicElements", BooleanValue (true));
  ...
  Config::SetDefault ("ns3::FriisPropagationLossModel::Frequency", DoubleValue(centerFrequency));
  ...
  mmwaveHelper->SetUePhasedArrayModelAttribute("NumColumns", UintegerValue(numAntennasMcUe));
  ...
  Config::SetDefault ("ns3::MmWaveEnbPhy::TxPower", DoubleValue (rsu_txPower));
  Config::SetDefault ("ns3::MmWaveUePhy::TxPower", DoubleValue (ue_txPower));
  ...
  ueNodes.Get (1)->GetObject<MobilityModel>()->SetPosition (Vector (10, 4.5, 2));
  ueNodes.Get(1)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(45/3.6, 0, 0));
  ...
  enbPositionAlloc->Add (Vector (110, 10, mmWaveRSUHeight));
  // enbPositionAlloc->Add (Vector (140, -10, mmWaveRSUHeight));
  // enbPositionAlloc->Add (Vector (170, 10, mmWaveRSUHeight));
  // enbPositionAlloc->Add (Vector (200, 10, mmWaveRSUHeight));
  // enbPositionAlloc->Add (Vector (230, -10, mmWaveRSUHeight));
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      OnOffHelper onOffAppUrllc ("ns3::UdpSocketFactory", InetSocketAddress (ueIpIface.GetAddress (u), 1234));
